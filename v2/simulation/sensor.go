...
type SensorContext struct {
	...
	// Cached per FeedForward call; heading doesn't change during sensor evaluation.
	FwdX, FwdY   float32
	HalfFOVCosSq float32 // c.halfFOVCos²
}
...
func (c *Creature) UpdateSensorContext(world *world.World, p *Population, params *Parameters) {
	...
	c.Sensors.SightFoodIDs = c.SightFoliageBuffer
...
func (c Creature) Sensor(sensorID byte, w *world.World, p *Population, ctx *SensorContext, simStep int, params *Parameters) float32 {
	...
	case LOC_X:
		output = float32(c.Loc.X/float32(params.World.Width))*2 - 1

	case LOC_Y:
		output = float32(c.Loc.Y/float32(params.World.Height))*2 - 1
	case OSC1:
		period := c.Clock
		if period < 2 {
			period = 2
	...
	case STOMACH_RATE:
		if params.Metabolism.DigestionRate > 0 {
			output = float32((float64(c.LastStomach) - float64(c.Stomach)) / params.Metabolism.DigestionRate)
		}
...
func calculateFoodDensityFwd(c Creature, w *world.World, ctx *SensorContext) float32 {
	...
	for _, id := range ctx.SightFoodIDs {
		...
		if rawDot < 0 || rawDot*rawDot < halfFOVCosSq*d2 {
			continue
		}
...
func calculateSightPopCentroid(c Creature, w *world.World, p *Population, ctx *SensorContext) float32 {
	...
	for _, id := range ctx.SightCreatureIDs {
		...
		rawDot := fwdX*dx + fwdY*dy
		if rawDot < 0 || rawDot*rawDot < halfFOVCosSq*distSq {
			continue
		}
...
func calculateDistanceToClosestKin(c Creature, p *Population, ctx *SensorContext, params *Parameters) float32 {
	...
	for i, id := range ctx.SightCreatureIDs {
		...
		if ctx.SightCreatureSims[i] < params.Reproduction.MinSimilarity {
			continue
		}
...
func calculateWallProximity(c Creature, w *world.World, params *Parameters) float32 {
	...
	minDistSq := math.Min(cx, params.World.Width-cx)
	minDistSq = minDistSq * minDistSq

	minYDist := math.Min(cy, params.World.Height-cy)
