...
func PassedSurvivalCriteria(c *Creature, s *Simulation, challenge ChallengeType) bool {
	switch challenge {
	case LeftSurvive:
		return float64(c.Loc.X) < s.Params.WorldWidth/2

	case FarLeftSurvive:
		return float64(c.Loc.X) < s.Params.WorldWidth/10
	...
	case Groups:
		minNeighbours := 4
		radius := float32(4.0)
		isBorder := float64(c.Loc.X) < 1 || float64(c.Loc.X) >= s.Params.WorldWidth-1 ||
			float64(c.Loc.Y) < 1 || float64(c.Loc.Y) >= s.Params.WorldHeight-1
		if isBorder {
			return false
		}
		nearEdge := float64(c.Loc.X) < 5 || float64(c.Loc.X) > s.Params.WorldWidth-6 ||
			float64(c.Loc.Y) < 5 || float64(c.Loc.Y) > s.Params.WorldHeight-6
		if nearEdge {
			return false
		}
	...
	case Center:
		cx := float32(s.Params.WorldWidth / 2)
		cy := float32(s.Params.WorldHeight / 2)
		radius := float32(50.0)
		dx := c.Loc.X - cx
		dy := c.Loc.Y - cy
