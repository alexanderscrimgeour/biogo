...
func PassedSurvivalCriteria(c *Creature, s *Simulation, challenge ChallengeType) bool {
	switch challenge {
	case LeftSurvive:
		return c.Loc.X < s.Params.WorldWidth/2

	case FarLeftSurvive:
		return c.Loc.X < s.Params.WorldWidth/10
	...
	case Groups:
		minNeighbours := 4
		radius := 4.0
		isBorder := c.Loc.X < 1 || c.Loc.X >= s.Params.WorldWidth-1 ||
			c.Loc.Y < 1 || c.Loc.Y >= s.Params.WorldHeight-1
		if isBorder {
			return false
		}
		nearEdge := c.Loc.X < 5 || c.Loc.X > s.Params.WorldWidth-6 ||
			c.Loc.Y < 5 || c.Loc.Y > s.Params.WorldHeight-6
		if nearEdge {
			return false
		}
	...
	case Center:
		cx := s.Params.WorldWidth / 2
		cy := s.Params.WorldHeight / 2
		radius := 50.0
		dx := c.Loc.X - cx
		dy := c.Loc.Y - cy
