...
func HeadingToVec(heading float32) (float32, float32) {
	return float32(math.Cos(float64(heading))), float32(math.Sin(float64(heading)))
}
...
func CosSimilarity(dx1, dy1, dx2, dy2 float32) float32 {
	d1 := float64(dx1)*float64(dx1) + float64(dy1)*float64(dy1)
	d2 := float64(dx2)*float64(dx2) + float64(dy2)*float64(dy2)
	mag1 := math.Sqrt(d1)
	mag2 := math.Sqrt(d2)
	if mag1 == 0 || mag2 == 0 {
		return 1
	}
	cos := (float64(dx1)*float64(dx2) + float64(dy1)*float64(dy2)) / (mag1 * mag2)
	if cos < -1 {
		return -1
	}
	if cos > 1 {
